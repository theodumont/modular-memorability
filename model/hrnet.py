...
class CustomBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, midplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = BatchNorm2d(midplanes, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(midplanes, planes, stride)
...
class CustomVBlock(nn.Module):
    def __init__(self, input_dim, fc_dim):
        super(CustomVBlock, self).__init__()
        self.layer1 = self._make_layer(CustomBasicBlock, input_dim, input_dim//2, input_dim//4, stride=2)
        self.layer2 = self._make_layer(CustomBasicBlock, input_dim//4, input_dim//8, input_dim//16, stride=2)
        self.linear = nn.Linear(fc_dim, 1)
        self.sigmoid = nn.Sigmoid()

    def _make_layer(self, block, inplanes, midplanes, planes, stride=2):
        norm_layer = nn.BatchNorm2d
        downsample = nn.Sequential(
                conv1x1(inplanes, planes * block.expansion, stride=2*stride),
                norm_layer(planes * block.expansion),
        )
        layer = block(inplanes=inplanes, midplanes=midplanes, planes=planes, stride=stride, downsample=downsample)
        return layer
...
def custom_vblock(pretrained=False, **kwargs):
    model = CustomVBlock(**kwargs)
    if pretrained:
...
class CustomDownsampleTo1(nn.Module):

    def __init__(self):
        super(CustomDownsampleTo1, self).__init__()
        self.downsample = nn.AvgPool2d(64)
...
def custom_downsample_to_1(pretrained=False, **kwargs):
    model = CustomDownsampleTo1(**kwargs)
    if pretrained:
        raise NotImplementedError
...
class SegmentationModule(nn.Module):
    ...
    def forward(self, input):
        pred = self.decoder(self.encoder(input))
...
class ModelBuilder:
    ...
    @staticmethod
    def build_encoder(arch='hrnetv2', fc_dim=512, weights=''):
        pretrained = True if len(weights) == 0 else False
        arch = arch.lower()
        if arch == 'hrnetv2':
            net_encoder = hrnetv2(pretrained=pretrained)
        else:
    ...
    def build_decoder(arch='c1',
        ...
        if arch == 'custom_downsample_to_1':
            net_decoder = custom_downsample_to_1()
        else:
            raise Exception('Architecture undefined!')
...
def build_hrnet(decoder_arch):
    net_encoder = ModelBuilder.build_encoder(
        arch='hrnetv2',
        fc_dim=720,
        weights=os.path.join(cfg.DIR.ROOT, 'model/pretrained_weights/ade20k-hrnetv2-c1/encoder_epoch_30.pth'))
